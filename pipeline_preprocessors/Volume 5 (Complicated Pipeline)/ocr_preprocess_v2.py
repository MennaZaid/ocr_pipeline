...
class Config:
    ...
    remove_border_junk: bool = True
    faint_recovery: bool = True
    reconstruct: bool = False   # bridge fragmented strokes (document-dependent)
    min_contrast: int = 15      # grey levels below local paper to call something ink
    strong_contrast: int = 35   # levels that make a pixel a confident seed
    thin: bool = False
    thin_guard: float = 0.12  # strip stamps / frames / signature scrawl
    dewarp: bool = False
...
class Result:
    ...
    final_inverted: np.ndarray | None = None
...
def reconstruct_strokes(bw, m, log):
    """
    OPT-IN stroke reconstruction for shattered photocopy text.

    Anisotropic closing: WIDE and SHORT, sized from stroke width, so it bridges
    gaps along the Arabic baseline without welding one line to the next.

    Measured warning: on the test scan this helped in exactly one of seven
    parameter cells and hurt in the rest. Fragments are not always collinear,
    and closing joins whatever is nearby -- including the wrong letters. Test it
    per document type with --ocr before enabling it in a batch.
    """
    sw = max(m["stroke_w"], 2.0)
    kx = max(3, int(round(sw * 1.25)))
    ky = max(1, int(round(sw * 0.5)))
    ker = cv2.getStructuringElement(cv2.MORPH_RECT, (kx, ky))
    ink = 255 - bw
    out = 255 - cv2.morphologyEx(ink, cv2.MORPH_CLOSE, ker, iterations=1)
    m2 = text_metrics(out)
    log.append(f"    stroke reconstruction: kernel {kx}x{ky} -> "
               f"components {m['n_comp']} -> {m2['n_comp']}")
    return out, m2


def paper_level(gray, text_h):
    """
    Estimate the local PAPER brightness with a morphological closing sized from
    text height. Referencing ink against local paper (rather than a local mean)
    is what makes very faint text measurable: a stroke 18 levels below its own
    paper is detectable even when its absolute value is 237.
    """
    k = int(max(15, text_h * 2.5)) | 1
    ker = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
    return cv2.GaussianBlur(cv2.morphologyEx(gray, cv2.MORPH_CLOSE, ker), (0, 0), k / 3.0)


def binarize_faint_aware(gray, m, cfg, log):
    """
    FAINT-TEXT RECOVERY — hysteresis on paper-referenced contrast.

    The problem with one global threshold: a setting loose enough to catch text
    at 10% of full contrast also promotes paper grain to ink. Hysteresis breaks
    that trade-off.

      strict  = confident ink  (>= strong_contrast below paper, OR Sauvola ink)
      loose   = candidate ink  (>= min_contrast below paper)
      keep a loose component if it is SEEDED by strict ink, or if it is
      TEXT-SHAPED on its own -- the second rule matters because an entire line
      can be faint, with no strong pixel anywhere to seed from.

    Grain is rejected by size (below ~0.3 x stroke_w^2 it cannot be a glyph part
    or an Arabic dot), which is why sensitivity can go high without noise.
    """
    ...
    strict = ((drop >= cfg.strong_contrast) | sv_ink).astype(np.uint8)
    loose = (drop >= cfg.min_contrast).astype(np.uint8)

    n, lbl, st, _ = cv2.connectedComponentsWithStats(loose, 8)
    seeded = np.zeros(n, bool)
    seeded[np.unique(lbl[strict > 0])] = True

    min_area = max(3.0, 0.12 * sw * sw)   # relaxed: faint text arrives as small fragments
    ...
    for i in range(1, n):
        ...
        if seeded[i]:
            ...
            keep[i] = True                 # fully faint but glyph-shaped
            rescued += 1

    out = np.isin(lbl, np.where(keep)[0])
    bw = np.where(out, 0, 255).astype(np.uint8)
    mm = text_metrics(bw)
    log.append(f"    faint recovery: min_contrast={cfg.min_contrast} "
               f"strong={cfg.strong_contrast} -> ink {100*mm['ink']:.2f}% "
               f"({rescued} components rescued with no strong seed)")
    return bw, cfg.min_contrast, mm


def binarize_to_target_orig(gray, text_h, cfg, log):
    """
...
# --------------------------------------------------------------------------- #
# v3: multi-stage noise filtering, contrast, polarity
# --------------------------------------------------------------------------- #
def filter_isolated(bw, m, log, factor=3.0):
    """
    PASS 2 — isolation by neighbourhood density.
    A real glyph fragment always has other ink within about one text height
    (its own letter body, the next letter, the line above). A scanner speck in a
    margin does not. Measured with a box filter instead of per-component
    distances, so it stays fast on pages with thousands of components.
    """
    ink, n, lbl, st, cent = component_stats(bw)
    if n <= 1:
        return bw
    win = int(max(9, m["text_h"] * 1.5)) | 1
    dens = cv2.boxFilter(ink.astype(np.float32), cv2.CV_32F, (win, win), normalize=True)
    limit = factor * (m["stroke_w"] ** 2)
    out, killed = ink.copy(), 0
    for i in range(1, n):
        if st[i, cv2.CC_STAT_AREA] > limit:
            continue
        cy, cx = int(cent[i][1]), int(cent[i][0])
        local = dens[max(cy - 1, 0):cy + 2, max(cx - 1, 0):cx + 2].mean()
        if local < 0.012:                      # <1.2% ink in a text-height window
            out[lbl == i] = 0
            killed += 1
    log.append(f"    isolation filter: removed {killed} sparse components")
    return np.where(out > 0, 0, 255).astype(np.uint8)


def remove_streaks(bw, m, log):
    """
    PASS 3 — scanner streaks and dust hairs.
    Deliberately conservative: Arabic kashida (ـــ) is a long thin horizontal
    stroke, so only components that are BOTH extremely elongated (aspect > 15)
    AND thinner than the measured stroke width are removed.
    """
    ink, n, lbl, st, _ = component_stats(bw)
    out, killed = ink.copy(), 0
    for i in range(1, n):
        ...
        thin = min(h, w)
        long_ = max(h, w)
        if thin <= max(1, m["stroke_w"] * 0.6) and long_ > 15 * max(thin, 1):
            out[lbl == i] = 0
            killed += 1
    log.append(f"    streak filter: removed {killed} hairline components")
    return np.where(out > 0, 0, 255).astype(np.uint8)


def remove_border_junk(bw, m, log, margin_frac=0.02):
    """
    PASS 4 — scan edge artefacts.
    Removes components that touch the image border AND are far longer than any
    glyph: black scanner edges, the page frame, torn-edge shadows.
    """
    H, W = bw.shape[:2]
    ...
    for i in range(1, n):
        ...
        oversized = max(w, h) > 6 * m["text_h"]
        if touches and oversized:
            out[lbl == i] = 0
            killed += 1
    log.append(f"    border filter: removed {killed} edge/frame components")
    return np.where(out > 0, 0, 255).astype(np.uint8)


def clean_binary(bw, m, cfg, log):
    """PASS 1-4 stacked. Each pass is size-relative to the measured page."""
    log.append("  noise filtering:")
    bw = remove_speckle(bw, m, log)
    bw = filter_isolated(bw, m, log)
    bw = remove_streaks(bw, m, log)
    if cfg.remove_border_junk:
        bw = remove_border_junk(bw, m, log)
    return bw


def guarded_thin(bw, m, cfg, log):
    """
    OPTIONAL stroke thinning, for photocopied documents whose strokes are fat in
    the SOURCE. Thresholding cannot fix that — once the scan is bilevel, stroke
    width is baked in and no choice of Sauvola k will change it. Only morphology
    can, and on cursive Arabic that is genuinely risky: erosion both separates
    welded letters (good) and snaps thin joins (bad), and the two are hard to
    tell apart from component counts alone.

    So: measure, apply, and revert if fragmentation exceeds the guard.
    """
    ink = 255 - bw
    er = cv2.erode(ink, cv2.getStructuringElement(cv2.MORPH_CROSS, (3, 3)), 1)
    out = 255 - er
    m2 = text_metrics(out)
    growth = m2["n_comp"] / max(m["n_comp"], 1) - 1.0
    if growth > cfg.thin_guard or m2["ratio"] < RATIO_LO:
        log.append(f"    thinning: REVERTED (components +{100*growth:.0f}%, "
                   f"ratio {m2['ratio']:.3f})")
        return bw, m
    log.append(f"    thinning: applied (ratio {m['ratio']:.3f} -> {m2['ratio']:.3f}, "
               f"components +{100*growth:.0f}%)")
    return out, m2


def maximize_contrast(gray, log):
    """
    Full-range linear stretch on the GREYSCALE image, before binarization.
    Worth being blunt about the binary outputs: a bilevel image already has the
    maximum possible contrast (values are exactly 0 and 255). Nothing can raise
    it further. This stage is what actually widens the black/white gap, and it
    runs while the image still has grey levels to work with.
    """
    ...
    log.append(f"    contrast stretch: [{lo:.0f},{hi:.0f}] -> [0,255]")
    return out


def polarity_pair(bw):
    """
    Returns (black_on_white, white_on_black).
    black_on_white -> Tesseract, EasyOCR, PaddleOCR, any classical engine.
    white_on_black -> morphology, connected-component work, and CNN/ViT models
                      trained with ink as the positive class (MNIST convention).
    """
    bow = bw if (bw < 128).mean() < 0.5 else cv2.bitwise_not(bw)
    return bow, cv2.bitwise_not(bow)
...
def process(name, bgr, cfg, has_text=False) -> Result:
    ...
    S["03_upscaled"] = gray
    gray = maximize_contrast(gray, L)
    S["03b_contrast_stretched"] = gray
    ...
    # CLAHE actively HURTS faint recovery: it equalises local contrast, which is
    # exactly the signal (grey levels below local paper) the recovery measures,
    # and it lifts paper grain into the ink range. Skip it in that mode.
    if cal["separation"] < 110 and not cal["is_bilevel"] and not cfg.faint_recovery:
    ...
    cal2 = calibrate(gray)
    if cfg.faint_recovery:
        need = 0.0                 # the hysteresis binarizer handles faint ink itself
    else:
    ...
    if cfg.faint_recovery:
        bw, k_used, m = binarize_faint_aware(gray, m0, cfg, L)
    else:
        bw, k_used, m = binarize_to_target_orig(gray, m0["text_h"], cfg, L)
    S["09_binarized"] = bw
    ...
    bw = clean_binary(bw, m, cfg, L)
    S["12_noise_filtered"] = bw
    ...
    if cfg.reconstruct:
        m = text_metrics(bw)
        bw, m = reconstruct_strokes(bw, m, L)
        S["15c_reconstructed"] = bw

    if cfg.thin:
        m = text_metrics(bw)
        bw, m = guarded_thin(bw, m, cfg, L)
        S["15b_thinned"] = bw
    ...
    bow, wob = polarity_pair(bw)
    res.final = bow
    res.final_inverted = wob
    S["18_FINAL_black_on_white"] = bow
    S["19_FINAL_white_on_black"] = wob
...
def main():
    ...
    ap.add_argument("--sensitivity", type=float, default=0.9,
                    help="0=conservative .. 1=catch text at ~5%% of full contrast")
    ap.add_argument("--min-contrast", type=int, default=None,
                    help="grey levels below local paper to call ink (overrides --sensitivity)")
    ap.add_argument("--no-faint-recovery", action="store_true")
    ap.add_argument("--reconstruct", action="store_true",
                    help="bridge fragmented strokes (test per document -- can hurt)")
    ap.add_argument("--thin", action="store_true",
                    help="try to thin fat photocopy strokes (guarded, reverts on damage)")
    ap.add_argument("--keep-border", action="store_true",
                    help="do NOT strip page frames / scanner edges")
    ...
    cfg = Config(dpi=a.dpi, boost=a.boost, sauvola_k=a.sauvola_k,
                 ...
                 drop_graphics=a.drop_graphics, thin=a.thin,
                 remove_border_junk=not a.keep_border,
                 faint_recovery=not a.no_faint_recovery, reconstruct=a.reconstruct,
                 min_contrast=(a.min_contrast if a.min_contrast is not None
                               else int(round(30 - 22 * np.clip(a.sensitivity, 0, 1)))))
    ...
    for name, img, has_text in collect_inputs(src, cfg) or []:
        ...
        if a.calibrate:
            continue
        imwrite_unicode(out / f"{name}_black_on_white.png", res.final)
        imwrite_unicode(out / f"{name}_white_on_black.png", res.final_inverted)
        if a.save_stages:
